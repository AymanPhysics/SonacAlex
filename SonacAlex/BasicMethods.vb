...
Public Class BasicMethods
    ...
    Public Function ToStrDate2(ByVal d As Object) As String
        Try
    ...
    Public Function AccNoShowHelpCombo(ByVal txtId As ComboBox, ByVal e As System.Windows.Input.MouseButtonEventArgs, Optional ByVal SubType As Integer = 1, Optional ByVal LinkFile As Integer = 0, Optional HideLinkedAccs As Boolean = False, Optional WithoutId2 As Boolean = True) As Boolean
        WithoutId2 = False

        Dim s As String = IIf(HideLinkedAccs, -1, 0)
        Return ShowHelp("Accounts", txtId, e, "select cast(Id as varchar(100)) ID,Name from Chart where " & IIf(WithoutId2, "Id not in(select Id2 from chart) and ", "") & "(SubType=" & SubType & " or " & SubType & "=-1) and (LinkFile=" & LinkFile & " or " & LinkFile & "=" & s & ")")
    End Function
    ...
    Sub AccNoLostFocusGrid(ByVal Cell As Forms.DataGridViewCell, ByVal txtName As Label, Optional ByVal SubType As Integer = 1, Optional ByVal LinkFile As Integer = 0)
        If Val(Cell.Value) = 0 Or Cell.ReadOnly Then
            ...
            Return
        End If
        Dim dt As DataTable = ExecuteAdapter("select Name from Chart where Id='" & Cell.Value.ToString & "'")
        If dt.Rows.Count = 0 Then
            ShowMSG("Invalid Id")
            Cell.Value = ""
            txtName.Content = ""
        ...
        dt = ExecuteAdapter("select Name from Chart where Id='" & Cell.Value.ToString & "' and (SubType=" & SubType & " or " & SubType & "=-1) and (LinkFile=" & LinkFile & " or " & LinkFile & "=0)")
        If dt.Rows.Count = 0 Then
            If SubType = 1 Then
                ShowMSG("Please, Select a Valid Account")
            ElseIf SubType = 0 Then
    ...
    Public Function AccNoShowHelpGrid(ByVal Column1 As Forms.DataGridViewCell, Column2 As Forms.DataGridViewCell, ByVal e As System.Windows.Forms.KeyEventArgs, Optional ByVal SubType As Integer = 1, Optional ByVal LinkFile As Integer = 0, Optional HideLinkedAccs As Boolean = False) As Boolean
        Dim s As String = IIf(HideLinkedAccs, -1, 0)
        Return ShowHelpGrid("Accounts", Column1, Column2, e, "select cast(Id as varchar(100)) ID,Name from Chart where (SubType=" & SubType & " or " & SubType & "=-1) and (LinkFile=" & LinkFile & " or " & LinkFile & "=" & s & ")")
    End Function
    ...
    Public Function ShowHelp(ByVal Header As String, ByVal txtId As TextBox, ByVal txtName As TextBox, ByVal e As System.Windows.Input.KeyEventArgs, ByVal Statement As String, Optional ByVal TableName As String = "", Optional MyFields() As String = Nothing, Optional MyValues() As String = Nothing, Optional ByVal FirstColumn As String = "", Optional ByVal SecondColumn As String = "") As Boolean
        If e Is Nothing OrElse e.Key = System.Windows.Input.Key.F1 Then
            ...
            If hh.SelectedId.ToString = "0" Then Return False
    ...
    Public Function ShowHelp(ByVal Header As String, ByVal txtId As ComboBox, ByVal e As System.Windows.Input.MouseButtonEventArgs, ByVal Statement As String, Optional ByVal TableName As String = "", Optional MyFields() As String = Nothing, Optional MyValues() As String = Nothing, Optional ByVal FirstColumn As String = "", Optional ByVal SecondColumn As String = "") As Boolean
        If Not e Is Nothing Then
            ...
            txtId.SelectedValue = hh.SelectedId.ToString
            Return True
        Else
            Return False
        End If
    ...
    Public Function ShowHelpGrid(ByVal Header As String, ByVal Column1 As Forms.DataGridViewCell, ByVal Column2 As Forms.DataGridViewCell, ByVal e As Forms.KeyEventArgs, ByVal Statement As String, Optional ByVal TableName As String = "", Optional ByVal FirstColumn As String = "", Optional ByVal SecondColumn As String = "") As Boolean
        If e.KeyCode = Forms.Keys.F1 Then
            ...
            hh.TableName = TableName
            hh.ShowDialog()
            If hh.SelectedId.ToString = "0" Then Return True
            Column1.Value = hh.SelectedId
            If Not Column1 Is Column2 Then Column2.Value = hh.SelectedName
            Return True
        Else
    ...
    Sub LostFocus(ByVal txtId As TextBox, ByVal txtName As ComboBox, ByVal Statement As String, Optional IgnoreVisibility As Boolean = False, Optional IgnoreShowMSG As Boolean = False)
        If txtId.Text.Trim = "" Or txtId.Text.Trim = "0" Then
            txtId.Clear()
        ...
        If dt.Rows.Count = 0 Then
            ...
            Return
        End If
        txtName.SelectedValue = dt.Rows(0)(0).ToString
    End Sub
    ...
    Public Function Save(ByVal ID() As String, ByVal IDValue() As String, Optional ByVal State As SaveState = SaveState.All, Optional TableName As String = Nothing) As Boolean
        ...
        Try
            ...
            If State <> SaveState.Insert Then
                MyCmd.CommandText = "IF Exists(Select *  From " & TableName & " T Where " & ID(0) & "='" & IDValue(0) & "'"
                For i As Integer = 1 To ID.Length - 1
                    MyCmd.CommandText &= " and " & ID(i) & "='" & IDValue(i) & "'"
                Next

                MyCmd.CommandText &= " " & AppendWhere & ")"
                MyCmd.CommandText &= " Update " & TableName & "  Set UserName='" & Md.UserName & "',MyGetDate=GetDate(),"
            ...
            If State <> SaveState.Update Then
                MyCmd.CommandText &= "Insert into " & TableName & "(UserName,MyGetDate," & Table_Fields() & " ) Values ('" & Md.UserName & "',GetDate(),'"
                For i As Integer = 0 To Fields.Length - 1
    ...
    Public Sub SearchTable(ByVal ID() As String, ByVal IDValue() As String, Cbo As ComboBox, Optional SpName As String = "", Optional UsedControls() As Control = Nothing, Optional OrderByStatement As String = "", Optional IncludeDate As Boolean = False)
        Try
            ...
            FillCombo(str, Cbo)
            Cbo.SelectedIndex = 0 ' Cbo.Items.Count - 1
            Cbo.Focus()
        Catch ex As Exception
            If ex.InnerException Is Nothing Then ShowMSG(ex.ToString().Trim) Else ShowMSG(ex.InnerException.Message)
        End Try
    ...
    Public Function SaveGrid(ByVal Grid As Forms.DataGridView, ByVal TableName As String, ByVal KeysFields() As String, ByVal KeysValues() As String, ByVal MyFields() As String, ByVal ColumnsNames() As String, ByVal Type() As VariantType, ByVal ColumnsKeys() As String, Optional ByVal LineField As String = "", Optional ByVal LineColumnName As String = "", Optional MasterAndDetailsAreInOneTable As Boolean = False) As Boolean
        ...
        For i As Integer = 0 To Grid.Rows.Count - 1 - IIf(Grid.AllowUserToAddRows, 1, 0)
            ...
            For x As Integer = 0 To ColumnsKeys.Length - 1
                If Grid.Rows(i).Cells(ColumnsKeys(x)).Value Is Nothing OrElse Grid.Rows(i).Cells(ColumnsKeys(x)).Value.ToString = "" OrElse Grid.Rows(i).Cells(ColumnsKeys(x)).Value.ToString = "0" Then GoTo EndFor 'OrElse Val(Grid.Rows(i).Cells(ColumnsKeys(x)).Value.ToString) = 0
            Next
...
A:
        Next
        If Focus AndAlso control(0).IsEnabled Then control(0).Focus()
    ...
    Public Function ToStrDateForGrid(ByVal d As Object) As String
        Try
            If d = Nothing Then Return ""
            Dim dd As DateTime = DateTime.Parse(d)
            If dd = "1900/01/01" Then Return ""
            Return dd.Year.ToString() & "/" & dd.Month.ToString().PadLeft(2, "0") & "/" & dd.Day.ToString().PadLeft(2, "0")
        Catch ex As Exception
            Return "1900/01/01"
        End Try
    ...
    Sub SetImage(Img As ImageBrush, MyUri As String)
        Try
            Dim bi3 As New BitmapImage
            bi3.BeginInit()
            bi3.UriSource = New Uri("pack://application:,,,/SonacAlex;component/Images/" & MyUri, UriKind.RelativeOrAbsolute)
            bi3.EndInit()
            Img.ImageSource = bi3
        Catch ex As Exception
            If ex.InnerException Is Nothing Then ShowMSG(ex.ToString().Trim) Else ShowMSG(ex.InnerException.Message)
...
A:
    ...
    Sub MakeGridCombo(G As MyGrid, cboHeader As String, cboName As String, statement As String, Optional MyFillWeight As Double = 200)
        ...
        cbo.DisplayStyle = Forms.DataGridViewComboBoxDisplayStyle.DropDownButton
        G.Columns.Add(cbo)
    ...
    Sub AddThousandSeparator(col As Forms.DataGridViewColumn, Optional dec As Integer = 2)
        col.ValueType = GetType(Decimal) ' or Integer, Single, etc.
        col.DefaultCellStyle.Format = "###,###,##0." & "".PadLeft(dec, "0") ' N(zero) for no digits to the right of the 
    End Sub

    Function AddThousandSeparator(txt As TextBox)
        txt.Text = Double.Parse(txt.Text).ToString("N", New Globalization.CultureInfo("en-US"))
    End Function
