...
Public Class RPT2
    ...
    Public Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles Button2.Click
        ...
        If SubAccNo.Visibility = Windows.Visibility.Visible AndAlso MyLinkFile = 0 AndAlso Val(SubAccNo.Text) = 0 AndAlso SubAccNo.IsEnabled AndAlso Flag <> 3 Then
            bm.ShowMSG("برجاء تحديد الحساب الفرعى")
            SubAccNo.Focus()
            Return
        End If

        If MyLinkFile > 0 AndAlso Val(SubAccNo.Text) = 0 AndAlso SubAccNo.Visibility = Windows.Visibility.Visible AndAlso Flag <> 3 Then
        ...
        rpt.paravalue = New String() {FromAccNo.SelectedValue, FromAccNo.Text, Val(SubAccNo.Text), SubAccName.Text, FromDate.SelectedDate, ToDate.SelectedDate, CType(Parent, Page).Title.Trim & " " & IIf(MainLinkFile.SelectedIndex > 0, MainLinkFile.Text, ""), IIf(Md.IsMyVeryFruits, 1, 0), 0, MyLinkFile, Val(SubAccNo.Text), RPTFlag1, 0, IIf(Flag2 = 1, 1, 0), 0, Val(WindowId.SelectedValue), Val(CostCenterId.SelectedValue), Val(CostCenterSubId.SelectedValue), FromAccNo.SelectedValue, ToAccNo.SelectedValue, Val(FromAnalysisId.SelectedValue), Val(ToAnalysisId.SelectedValue), Val(FromStatusId.SelectedValue), Val(ToStatusId.SelectedValue)}
        Select Case Flag
            Case 1
